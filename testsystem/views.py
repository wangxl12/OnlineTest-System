...
from .models import Paper, Score, CheckIn, CheckInInfo
import hashlib
from . import models
from . import forms
...
def brokenline(request):
    ...
    return render(request, 'testsystem/brokenline.html', locals())

def pie(request):
    # there is no real data now, you can generate by yourself
    # 
    # extend your functions here
    return render(request, 'testsystem/pie.html', locals())

def histogram(request):
    # there is no real data now, you can generate by yourself
    # 
    # extend your functions here
    return render(request, 'testsystem/histogram.html', locals())
...
def index(request):
    ...
    paper_nofinish = []
    # 签到
    checkin_list = CheckIn.objects.order_by('-id')
    checkin_count = CheckIn.objects.all().count()
    checkin_finish = CheckInInfo.objects.filter(user_id=request.session['user_id']).values_list('id',
                                                                                                'checkin_value', 
                                                                                                'answer',
                                                                                                'date')
    checkin_nofinish = []
    ...
    for i in checkin_list:
        flag = False
        for j in checkin_finish:
            if i.id == j[1]:
                flag = True
        if not flag:
            checkin_nofinish.append(i)
            
    return render(request, 'testsystem/index.html',
                  {'paper_nofinish': paper_nofinish, 'paper_count': paper_count, 'score_list': score_list,
                   'checkin_count': checkin_count, 'checkin_nofinish': checkin_nofinish, 'checkin_finish': checkin_finish})

def checkin(request):
    render(request, 'testsystem/checkin.html')
    print(request.method)
    if request.method == "GET":
        # username = request.POST.get('username')
        checkin = request.session.get('answer')
        message = '请检查填写的内容！'
        if checkin:  # 确保用户名和密码都不为空
            try:
                checkin = models.CheckIn.objects.get(random_number=checkin)
                print("入口1")
                return redirect('/index/')
            except:
                message = '请输入正确的签到码！'
                return render(request, 'testsystem/checkin.html', {'message': message})
        else:
            print("入口2")
            return render(request, 'testsystem/checkin.html', {'message': message})
    return render(request, 'testsystem/checkin.html')
...
def login(request):
    ...
    if request.method == "POST":
        ...
        message = '请检查填写的内容！'
        if password and idcard:  # 确保用户名和密码都不为空
            # 用户名字符合法性验证
            # 更多的其它验证.....
            try:
...
def info(request):
    render(request, 'testsystem/info.html')
