...
admin.site.site_header = '教师登录界面'
admin.site.site_title = '教师登录界面'
        ...
        for i in lists:
            if i['题型'] == '单选题':
                ...
                for j in list('ABCDEF'):
                    if '选项' + j in i.keys():
                        new_sc_choice = SC_choice(choice_symbol=j, choice_text=i['选项' + j], question=new_sc_question,
                                                  paper=new_paper)
                        new_sc_choice.save()
            elif i['题型'] == '多选题':
                ...
                for j in list('ABCDEF'):
                    if '选项' + j in i.keys():
                        new_mc_choice = MC_choice(choice_symbol=j, choice_text=i['选项' + j], question=new_mc_question,
                                                  paper=new_paper)
                        new_mc_choice.save()
            else:
                new_blank_question = Blank_question(blank_text=i['问题描述'], blank_solution=i['正确答案'], value=i['分值'],
                                                    paper=new_paper)
                new_blank_question.save()


# class CInAdmin(admin.TabularInline):
#     model = CheckIn
#     extra = 4
class CheckInInfoAdmin(admin.ModelAdmin):
    list_display = ('date', 'answer')
    list_display_links = ('date', 'answer')

class CheckInAdmin(admin.ModelAdmin):
    list_display = ('id', 'checkin_text', 'time', 'date', 'random_number')
    list_display_links =('id', 'checkin_text', 'time', 'date', 'random_number')
    search_fields = ['checkin_text']
    list_filter = ['date']
    # inlines = [CInAdmin]
    # checkin = CheckIn()
    # checkin.save()
...
# admin.site.register(SC_choice)  # 单选题选项
# admin.site.register(MC_choice)  # 多选题选项
...
admin.site.register(CheckInInfo, CheckInInfoAdmin)
